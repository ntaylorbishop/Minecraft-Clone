...
float ComputePerlinNoise2D(const Vector2& position, float perlinNoiseGridCellSize, int numOctaves, float persistence)
{
	...
	for (int octaveNumber = 0; octaveNumber < numOctaves; ++octaveNumber)
	{
		...
		float southwestDot = DotProductTemp(southwestNoiseGradient, perlinPositionUV);
		float southeastDot = DotProductTemp(southeastNoiseGradient, Vector2(perlinPositionAntiUV.x, perlinPositionUV.y));
		float northeastDot = DotProductTemp(northeastNoiseGradient, perlinPositionAntiUV);
		float northwestDot = DotProductTemp(northwestNoiseGradient, Vector2(perlinPositionUV.x, perlinPositionAntiUV.y));
