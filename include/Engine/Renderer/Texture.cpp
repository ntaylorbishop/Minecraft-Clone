...
STATIC map< string, Texture* >	Texture::s_textureRegistry;

Texture::Texture(const string& imageFilePath) : m_openglTextureID(0), m_texelSize(0, 0) {
...
STATIC Texture* Texture::GetTextureByName(const string& imageFilePath) {
	auto texture = s_textureRegistry.find(imageFilePath);

	if (texture == s_textureRegistry.end())
		return nullptr;
	else
...
STATIC Texture* Texture::CreateOrGetTexture(const string& imageFilePath) {
	...
	if (texture != s_textureRegistry.end())
		...
		s_textureRegistry.insert(std::pair<string, Texture*>(imageFilePath, new Texture(imageFilePath)));
		return texture;
	}
}
