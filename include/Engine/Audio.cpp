...
AudioChannelHandle AudioSystem::PlayLoop(SoundID soundID, float volumeLevel)
{
	unsigned int numSounds = m_registeredSounds.size();
	if (soundID < 0 || soundID >= numSounds)
		return nullptr;
...
void AudioSystem::PauseChannel(AudioChannelHandle channel) {
	if (channel != nullptr)
	{
		FMOD::Channel* fmodChannel = (FMOD::Channel*) channel;
...
void AudioSystem::UnpauseChannel(AudioChannelHandle channel) {
	if (channel != nullptr)
	{
		FMOD::Channel* fmodChannel = (FMOD::Channel*) channel;
...
void AudioSystem::ChangeVolumeOfChannel(AudioChannelHandle channel, float volume) {
	if (channel != nullptr) {
		FMOD::Channel* fmodChannel = (FMOD::Channel*) channel;
